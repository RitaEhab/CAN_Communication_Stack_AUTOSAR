...
typedef struct {
	...
	Can_PduType *CanIfTxPduRef;		// void* CanIfTxPduRef (PREVIOUS YEAR)
}CanIfTxPduCfg;
...
/*  This container contains the Txbuffer configuration. Multiple buffers with
	different sizes could be configured. If CanIfBufferSize equals 0, the CanIf
...
typedef struct {
	...
	/*  Reference to HTH, that defines the hardware object or the pool of hardware
	 	objects configured for transmission */
	CanIfHthCfg *CanIfBufferHthRef;
} CanIfBufferCfg;
