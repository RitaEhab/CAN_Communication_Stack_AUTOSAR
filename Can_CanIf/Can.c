...
void Can_Init( const Can_ConfigType* Config )
{
	...
	#endif
		{
			...
			/* SELECT CLOCK SOURCE */
			SET_BIT(CAN_MCR,31);				// Enter disable mode (MDIS = 1)
			CLEAR_BIT(CAN_CTRL1,13);			// Assign clock source (CLKSRC = 0)
			CLEAR_BIT(CAN_MCR,31);				// Enter enable mode (MDIS = 0)


			/* CHECK FREEZE MODE ENTRY */
			while (BIT_IS_CLEAR(CAN_MCR,27) && BIT_IS_CLEAR(CAN_MCR,24));	// (FRZACK == 0 && NTRDY == 0)
			...
			MB31_CS &= ~(0x0F000000);		// Control and Status word of all message buffers is initialized
												// CODE = INACTIVE
			// ^^^ TO BE REMOVED IF MY habd IS CORRECT ^^^
...
Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType* PduInfo )
{
	...
	if(Hth != Can_ConfigStructPtr->canHardwareObject[1]->CanObjectId) // Reham check here
	{
		/* Report to DET  */
		Det_ReportError(CAN_MODULE_ID, CAN_INSTANCE_ID,
...
#endif
	{
		...
		else
		{
			...
			/* SET ID & DATA */
			MB31_ID |= ((PduInfo->swPduHandle)<<18);
...
__interrupt void CanReceptionInterrupt()
{
	...
	while(BIT_IS_SET(MB31_CS,24)); // Reham Check this
